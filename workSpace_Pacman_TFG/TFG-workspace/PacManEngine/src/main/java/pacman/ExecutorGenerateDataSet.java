...
import pacman.game.dataSet.DataSetRecorder;
...
public class ExecutorGenerateDataSet {
	 ...
	 */
	public Stats[] runExperiment(Controller<MOVE> pacManController, GhostController ghostController, int trials,
			String description) {
	...
	public Stats[] runExperimentTicks(Controller<MOVE> pacManController, GhostController ghostController, int trials,
			String description) {
		Stats stats = new Stats(description);
	...
	public int runGame(Controller<MOVE> pacManController, GhostController ghostController, int delay) {
		...
		// Guardo los datos
		try {
			dataRecorder.saveDataToCsv("gameData", true);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	/**
	 * Run the game with time limit (asynchronous mode). Can be played with and
	 * without visual display of game states.
	 *
	 * @param pacManController The Pac-Man controller
	 * @param ghostController  The Ghosts controller
	 */
	 ...
	 */
	public Stats runGameTimedSpeedOptimised(Controller<MOVE> pacManController, GhostController ghostController,
			boolean fixedTime, String desc) {
		Game game = setupGame();
	 ...
	 * @return Stat score achieved by Ms. Pac-Man
	 */
	public Stats runGameTimeFixed(Controller<MOVE> pacManController, GhostController ghostController, int time,
			String desc) {
		Game game = setupGame();
	 ...
	 */
	public Stats runGameTimedRecorded(Controller<MOVE> pacManController, GhostController ghostController,
			String fileName) {
	...
	/**
	 * Run a game in asynchronous mode and notifies observer: the game waits until a
	 * move is returned. In order to slow thing down in case the controllers return
	 * very quickly, a time limit can be used. If fasted gameplay is required, this
	 * delay should be put as 0.
	 *
	 * @param pacManController The Pac-Man controller
	 * @param ghostController  The Ghosts controller
	 * @param delay            The delay between time-steps
	...
	public int runGame(Controller<MOVE> pacManController, GhostController ghostController, int delay,
		...
		while (!game.gameOver()) {
			...
			handlePeek(game);
			MOVE pacManMove = pacManController.getMove(getPacmanCopy(game), System.currentTimeMillis() + timeLimit);
			EnumMap<GHOST, MOVE> ghostsMove = ghostControllerCopy.getMove(getGhostsCopy(game),
	 ...
	 */
	public int runGame(Controller<MOVE> pacManController, GhostController ghostController, int delay,
			GameObserver observer, String title) {
		Game game = setupGame();
		...
		/*
		try {
			dataRecorder.saveData("gameData", true);
		} catch (IOException e) {
			e.printStackTrace();
		}
		*/
